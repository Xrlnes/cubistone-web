  ...
  >
    ...
    <div className="relative bg-white rounded-2xl overflow-hidden shadow-2xl">
      <div className="relative">
        ...
        {/* Floating Elements */}
        <div className="absolute -right-4 md:-right-6 -top-4 md:-top-6 w-16 h-16 md:w-24 md:h-24 bg-blue-500 rounded-full opacity-20 blur-xl group-hover:scale-150 transition-transform duration-500" />
        <div className="absolute -left-4 md:-left-6 -bottom-4 md:-bottom-6 w-16 h-16 md:w-24 md:h-24 bg-blue-500 rounded-full opacity-20 blur-xl group-hover:scale-150 transition-transform duration-500" />
      </div>

      <div className="p-4 md:p-8 relative">
        <div className="flex items-center gap-1 md:gap-3 text-blue-600 mb-2 md:mb-4">
          <span className="font-medium text-sm md:text-base">{author}</span>
          <span className="text-blue-300 text-xs md:text-base">•</span>
          <span className="text-gray-500 text-xs md:text-sm">{date}</span>
        </div>
        
        <h3 className="text-lg md:text-2xl font-bold text-gray-800 mb-2 md:mb-4 group-hover:text-blue-600 transition-colors">
          {title}
        </h3>

        <button className="flex items-center gap-1 md:gap-2 text-blue-600 font-semibold group/button text-sm md:text-base">
          Read More
          <svg className="w-4 h-4 md:w-5 md:h-5 group-hover/button:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const NewsModal = ({ news, onClose }) => (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden">
      ...
      <div className="absolute top-0 right-0 left-0 h-64 md:h-80 bg-gradient-to-t from-white via-white/50 to-transparent" />
      
      <div className="relative -mt-16 md:-mt-20 p-4 md:p-8 pt-0">
        <div className="flex items-center gap-1 md:gap-3 text-blue-600 mb-2 md:mb-4">
          <span className="font-medium text-sm md:text-base">{news.author}</span>
          <span className="text-blue-300 text-xs md:text-base">•</span>
          <span className="text-gray-500 text-xs md:text-sm">{news.date}</span>
        </div>
        
        <h2 className="text-2xl md:text-4xl font-bold text-gray-800 mb-4 md:mb-6">{news.title}</h2>
        <p className="text-gray-600 text-base md:text-lg leading-relaxed">{news.description}</p>

        <button 
          onClick={onClose}
          className="absolute top-2 md:top-4 right-2 md:right-4 p-2 text-gray-400 hover:text-gray-600 transition-colors"
        >
